...
/**************************************************************************/
// default timeout of one second
bool Adafruit_PN532::sendCommandCheckAck(uint8_t *cmd, uint8_t cmdlen, uint16_t timeout, CyMCP23016 expander) {
  uint16_t timer = 0;
...
bool Adafruit_PN532::readPassiveTargetID(uint8_t cardbaudrate, uint8_t * uid, uint8_t * uidLength, CyMCP23016 expander, uint16_t timeout) {
  ...
  if (!sendCommandCheckAck(pn532_packetbuffer, 3, timeout, expander))
  {
    #ifdef PN532DEBUG
      PN532DEBUGPRINT.println(F("No card(s) read"));
    #endif
