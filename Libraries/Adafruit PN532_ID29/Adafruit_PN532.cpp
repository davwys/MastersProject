...
/**************************************************************************/
/*!
    @brief  Instantiates a new PN532 class using software SPI, connected via MCP23016 IO Expander.

    @param  clk       SPI clock pin (SCK)
    @param  miso      SPI MISO pin
    @param  mosi      SPI MOSI pin
    @param  ss        SPI chip select pin (CS/SSEL)
    @param  expander        The CyMCP23016 expander object to use
*/
/**************************************************************************/
Adafruit_PN532::Adafruit_PN532(uint8_t clk, uint8_t miso, uint8_t mosi, uint8_t ss, CyMCP23016 expander):
  _clk(clk),
  _miso(miso),
  _mosi(mosi),
  _ss(ss),
  ...
  _hardwareSPI(false)
{
  expander.pinMode(_ss, OUTPUT);
  expander.digitalWrite(_ss, HIGH);
  pinMode(_clk, OUTPUT);
  pinMode(_mosi, OUTPUT);
  pinMode(_miso, INPUT);
}
...
/**************************************************************************/
/*!
    @brief  Setups the HW, using MCP23016 expander
*/
/**************************************************************************/
void Adafruit_PN532::begin(CyMCP23016 expander) {
  if (_usingSPI) {
    // SPI initialization
    if (_hardwareSPI) {
      SPI.begin();
    ...
    expander.digitalWrite(_ss, LOW);

    delay(4000);

    // not exactly sure why but we have to send a dummy command to get synced up
    pn532_packetbuffer[0] = PN532_COMMAND_GETFIRMWAREVERSION;
    sendCommandCheckAck(pn532_packetbuffer, 1, expander);
...
/**************************************************************************/
/*!
    @brief  Checks the firmware version of the PN5xx chip, using MCP23016 expander

    @returns  The chip's firmware version and ID
*/
/**************************************************************************/
uint32_t Adafruit_PN532::getFirmwareVersion(CyMCP23016 expander) {
  uint32_t response;

  pn532_packetbuffer[0] = PN532_COMMAND_GETFIRMWAREVERSION;

  if (! sendCommandCheckAck(pn532_packetbuffer, 1, expander)) {
    return 0;
  }

  // read data packet
  readdata(pn532_packetbuffer, 12, expander);
...
/**************************************************************************/
/*!
    @brief  Sends a command and waits a specified period for the ACK, using MCP23016 expander
...
/**************************************************************************/
// default timeout of one second
bool Adafruit_PN532::sendCommandCheckAck(uint8_t *cmd, uint8_t cmdlen, CyMCP23016 expander, uint16_t timeout) {
  uint16_t timer = 0;

  // write the command
  writecommand(cmd, cmdlen, expander);

  // Wait for chip to say its ready!
  if (!waitready(timeout, expander)) {
    return false;
  }
...
/**************************************************************************/
/*!
    @brief  Configures the SAM (Secure Access Module), using MCP23016 expander
*/
/**************************************************************************/
bool Adafruit_PN532::SAMConfig(CyMCP23016 expander) {
  pn532_packetbuffer[0] = PN532_COMMAND_SAMCONFIGURATION;
  pn532_packetbuffer[1] = 0x01; // normal mode;
  pn532_packetbuffer[2] = 0x14; // timeout 50ms * 20 = 1 second
  pn532_packetbuffer[3] = 0x01; // use IRQ pin!

  if (! sendCommandCheckAck(pn532_packetbuffer, 4, expander))
    return false;

  // read data packet
  readdata(pn532_packetbuffer, 8, expander);
...
/**************************************************************************/
/*!
    Waits for an ISO14443A target to enter the field, using MCP23016 expander
...
*/
/**************************************************************************/
bool Adafruit_PN532::readPassiveTargetID(uint8_t cardbaudrate, uint8_t * uid, uint8_t * uidLength, CyMCP23016 expander, uint16_t timeout) {
  pn532_packetbuffer[0] = PN532_COMMAND_INLISTPASSIVETARGET;
  pn532_packetbuffer[1] = 1;  // max 1 cards at once (we can set this to 2 later)
  pn532_packetbuffer[2] = cardbaudrate;

  if (!sendCommandCheckAck(pn532_packetbuffer, 3, expander, timeout))
  {
    #ifdef PN532DEBUG
      Serial.println(F("No card(s) read"));
    #endif
  ...
  // read data packet
  readdata(pn532_packetbuffer, 20, expander);
  // check some basic stuff
...
bool Adafruit_PN532::inDataExchange(uint8_t * send, uint8_t sendLength, uint8_t * response, uint8_t * responseLength) {
  ...
  if (!sendCommandCheckAck(pn532_packetbuffer,sendLength+2,1000)) {
    #ifdef PN532DEBUG
      Serial.println(F("Could not send APDU"));
    #endif
    return false;
  }
...
/**************************************************************************/
/*!
    Tries to read an entire 4-byte page at the specified address, using MCP23016 expander.
...
*/
/**************************************************************************/
uint8_t Adafruit_PN532::ntag2xx_ReadPage (uint8_t page, uint8_t * buffer, CyMCP23016 expander)
{
  // TAG Type       PAGES   USER START    USER STOP
  // --------       -----   ----------    ---------
  // NTAG 203       42      4             39
  ...
  /* Read the response packet */
  readdata(pn532_packetbuffer, 26, expander);
  #ifdef MIFAREDEBUG
    Serial.println(F("Received: "));
    Adafruit_PN532::PrintHexChar(pn532_packetbuffer, 26);
  #endif
...
*/
/**************************************************************************/
bool Adafruit_PN532::readack(CyMCP23016 expander) {
  uint8_t ackbuff[6];

  readdata(ackbuff, 6, expander);
...
/**************************************************************************/
/*!
    @brief  Return true if the PN532 is ready with a response, using MCP23016 expander.
*/
/**************************************************************************/
bool Adafruit_PN532::isready(CyMCP23016 expander) {
  if (_usingSPI) {
    // SPI read status and check if ready.
    #ifdef SPI_HAS_TRANSACTION
      if (_hardwareSPI) SPI.beginTransaction(PN532_SPI_SETTING);
...
/**************************************************************************/
/*!
    @brief  Waits until the PN532 is ready, using MCP23016 expander.

    @param  timeout   Timeout before giving up
*/
/**************************************************************************/
bool Adafruit_PN532::waitready(uint16_t timeout, CyMCP23016 expander) {
  uint16_t timer = 0;
  while(!isready(expander)) {
    if (timeout != 0) {
      timer += 10;
      if (timer > timeout) {
        #ifdef PN532DEBUG
...
/**************************************************************************/
/*!
    @brief  Reads n bytes of data from the PN532 via SPI or I2C, using MCP23016 expander.

    @param  buff      Pointer to the buffer where data will be written
    @param  n         Number of bytes to be read
*/
/**************************************************************************/
void Adafruit_PN532::readdata(uint8_t* buff, uint8_t n, CyMCP23016 expander) {
  if (_usingSPI) {
    // SPI write.
    #ifdef SPI_HAS_TRANSACTION
      if (_hardwareSPI) SPI.beginTransaction(PN532_SPI_SETTING);
...
/*!
    @brief  Writes a command to the PN532, automatically inserting the
            preamble and required frame details (checksum, len, etc.), using MCP23016 expander

    @param  cmd       Pointer to the command buffer
    @param  cmdlen    Command length in bytes
*/
/**************************************************************************/
void Adafruit_PN532::writecommand(uint8_t* cmd, uint8_t cmdlen, CyMCP23016 expander) {
  if (_usingSPI) {
    // SPI command write.
    uint8_t checksum;
